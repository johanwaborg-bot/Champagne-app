...
function WsetNotes({ notes, onChange, onScan }) {
  ...
  const reset = () => { setScan("idle"); setImg(null); setErr(""); };
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        {(scan === "preview" || scan === "error") && (
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <label style={{ flex: 1, padding: "11px 0", textAlign: "center", cursor: "pointer", fontFamily: serif, fontSize: 13, letterSpacing: 3, textTransform: "uppercase", borderRadius: 2, border: `1px solid ${LINE}`, color: GOLD_DEEP }}>Retake<input type="file" accept="image/*" onChange={e => { const f = e.target.files[0]; e.target.value = ""; pick(f); }} style={{ display: "none" }} /></label>
    ...
    <div style={{ marginBottom: 24 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <label style={{ background: "transparent", border: `1px solid ${LINE}`, borderRadius: 2, color: GOLD_DEEP, padding: "5px 11px", cursor: "pointer", fontFamily: serif, fontSize: 12, letterSpacing: 1.5, display: "inline-flex", alignItems: "center", gap: 5 }}>📷 SCAN<input type="file" accept="image/*" onChange={e => { const f = e.target.files[0]; e.target.value = ""; pick(f); }} style={{ display: "none" }} /></label>
        </div>
      </div>
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      {photo ? (
        ...
        <label style={{ display: "block", textAlign: "center", width: "100%", padding: "16px", border: `1.5px dashed ${LINE}`, borderRadius: 3, background: "rgba(255,255,255,0.3)", cursor: "pointer", fontFamily: serif, fontSize: 14, color: GOLD_DEEP, letterSpacing: 1, boxSizing: "border-box" }}>
          📷 &nbsp;Add Bottle Photo
          <input type="file" accept="image/*" onChange={e => { const f = e.target.files[0]; e.target.value = ""; pick(f); }} style={{ display: "none" }} />
        </label>
      )}
    </div>
  );
